...
import com.bea.remote.models.crm.CrmCunKuan;
 ...
 * @author fanglinhao
 */
public class CRMCunKuanViewFragment extends XCBaseHttpFragment{
	@ViewInject(R.id.webview)
	WebView webView;
	Map<String ,Object> map;
	List<CrmCunKuan> list;
	public CRMCunKuanViewFragment(Map<String ,Object> map) {
		this.map = map;
	}

	@Override
	protected View onCreateView(LayoutInflater inflater, ViewGroup group) {
		return inflater.inflate(R.layout.fm_grwdy_crm_cunkuan, null);
	}
	...
	private void requestData(){
		if(null == map)
			return;
		try {
			jsonRequest(102, "通用接口", JsonRemoteBO.reqParam(ConstDefined.crmCunKuanDetial, map));
		} catch (Exception e) {
			XCLog.e("crm存款详情请求错误信息", e.getMessage());
			e.printStackTrace();
		}
	}
	...
	public void onResponsSuccess(int actionTag, Object object) {
		if(102 == actionTag){
			try {
				list = XCJsonHelper.parseArray(object.toString(),CrmCunKuan.class);
				loadHtml();
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public class JsHelper
    {
        ...
        public void loadData(){
        	if(null == list){
            	list = new ArrayList<CrmCunKuan>();
        	}
        	final String json = XCJsonHelper.toJSONString(list);
        	webView.post(new Runnable() {
				@Override
				public void run() {
					webView.loadUrl("javascript:loadCunkuanData('"+json+"')");
				}
			});
        }
    }
