...
import com.bea.remote.models.crm.CrmDaikuan;
 ...
 * @author fanglinhao
 */
public class CRMDaikuanViewFragment extends XCBaseHttpFragment {
	@ViewInject(R.id.webview)
	WebView webView;
	Map<String ,Object> map;
	List<CrmDaikuan> list;
	public CRMDaikuanViewFragment(Map<String, Object> map) {
		this.map = map;
	}

	@Override
	protected View onCreateView(LayoutInflater inflater, ViewGroup viewGroup) {
		return inflater.inflate(R.layout.fm_grwdy_crm_daikuan, null);
	}
	...
	private void requestData(){
		if(null == map)
			return;
		try {
			jsonRequest(105, "通用接口", JsonRemoteBO.reqParam(ConstDefined.crmDaiKuanDetial, map));
		} catch (Exception e) {
	...
	public void onResponsSuccess(int actionTag, Object object) {
		if(105 == actionTag){
			try {
				list = XCJsonHelper.parseArray(object.toString(),CrmDaikuan.class);
				loadHtml();
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public class JsHelper
    {
        ...
        public void loadData(){
        	if(null == list)
        		list = new ArrayList<CrmDaikuan>();
        	final String json = XCJsonHelper.toJSONString(list);
        	webView.post(new Runnable() {
				@Override
				public void run() {
					webView.loadUrl("javascript:loadDaikuanData('"+json+"')");
				}
			});
        }
    }
